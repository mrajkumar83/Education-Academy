...
switch ($op) {
    case 'A':
        ...
        $store = $db->storeDetails('tb_lib_categories', $fields);
        $id = $db->newRowId;

        break;
    case 'E':
        $fields .=' , 	category_modifiedby = "' . $_SESSION['UID'] . '", category_modifieddate = "' . date(DATE_TIME_FORMAT) . '"';
        $db->storeDetails('tb_lib_categories', $fields, ' WHERE category_id = "' . $id . '"');
        break;

    case 'D':
        $db->delData('tb_lib_categories', ' category_id="' . $id . '"');
        break;
}
header('Location: ../manage_library_category.php?sts=' . $sts);
exit;