...
switch ($op) {
    case 'A':
        ...
        $store = $db->storeDetails('tb_vehcile_categories', $fields);
        $id = $db->newRowId;

        break;
    case 'E':
        $fields .=' , 	category_modifiedby = "' . $_SESSION['UID'] . '", category_modifieddate = "' . date(DATE_TIME_FORMAT) . '"';
        $db->storeDetails('tb_vehcile_categories', $fields, ' WHERE category_id = "' . $id . '"');
        break;

    case 'D':
        $db->delData('tb_vehcile_categories', ' category_id="' . $id . '"');
        break;
}
header('Location: ../manage_vehcile_category.php?sts=' . $sts);
exit;