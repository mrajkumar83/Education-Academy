...
class Spreadsheet_Excel_Reader {
	...
	function xl_arr($cols = '',$row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$arr = array();
		for($row=2;$row<=$this->rowcount($sheet);$row++) 
		{
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) 
			{
				...
				$colspan = (($cols == '') ? $this->colspan($row,$col,$sheet) : $cols);
	...
	// =========================================
	function storeTest($test_id,$row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		for($row=2;$row<=$this->rowcount($sheet);$row++) 
		{
			...
			$sql = "insert into tb_test_details(test_id,test_question,test_option1,
						...
						values(".$test_id.",'".$q_fields[1]."',
						'".$q_fields[2]."','".$q_fields[3]."','".$q_fields[4]."',
						'".$q_fields[5]."','".$q_fields[6]."','".$q_fields[7]."')";
					//	echo "<br><br>";
				mysql_query($sql);
	...
	function storeAttendance($att_date,$att_doc_id,$att_batch_id,$att_branch,$row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$out = "<table class=\"$table_class\" cellspacing=0>";
		/*if ($col_letters) 
		{
			$out .= "<thead>\n\t<tr>";
		...
		for($row=2;$row<=$this->rowcount($sheet);$row++) 
		{
			...
			$sql = "insert into tb_att_details(att_doc_id,student_id,student_name,
							attendance,date,att_batch_id,att_branch_id)
						values(".$att_doc_id.",'".$q_fields[0]."','".$q_fields[1]."'
						,'".$q_fields[2]."','".$att_date."','".$att_batch_id."','".$att_branch."')";
						//echo "<br><br>";
